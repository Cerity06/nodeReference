...
// name of the event is first argument, and the callback
...
// LOAD A PAGE WHEN CLIENT GOES TO PAGE INDEX
// DYNAMIC ROUTE: '/:id'
app.get('/index', (req, res) => {
  res.sendFile(path.join(process.cwd(), './index.html')); // can be a file and will display this on the index page
});
...
// QUERY
// '/index/api/query?search=5'
app.get('/index/api/query', (req, res) => {
...
**AVOID TO REPEAT CODE VALIDATION**

- It becomes a part of the pipeline as a validation step in the middleware.
- next is needed to go to the next middleware.
...
Those middleware are pre-something because they are launch before all the classic elements.

validate data: "npm i validator"

```ts
// VALIDATE IN SCHEMA: this only points to current doc on NEW document creation
validate: function (value) {
    return value < this.price;
},

// If needed, possible to use module validation
...
IMPORTANT: **FAT MODELS/THIN CONTROLLERS:**
Offload as much logic as possible into the models, and keep the controllers as simple and lean as possible.

# ERROR HANDLING WITH EXPRESS

**Google:** 'npm i ndb --save-dev'

**Script**: in package.json: "debug": "ndb server.js"

How to use:

- select line to stop the code and go step by step.
- Any change will be reflected in the original file.

## HANDLING UNHANDLED ROUTES

```ts
// To handle all other routes that are note managed by the other normal routes
// Possible to add at the end of the router document
...
**OPERAIONAL ERRORS:** problems that we can predict will happen at some point.

- Invalid path accessed or user input
- Failed to connect to server or database
- request timeout...

**PROGRAMMING ERRORS:** bugs introduced by developers into the code
(Promise, sync/async, properties, wrong types...)

**How to manage operational errors?** Use one central middleware check

Create a file in utils/appError.ts

1. **CREATE OUR OWN ERROR CLASS**: allows to better catch errors.
2. **MIDDLEWARE CATCH ERROR:** build a middleware function to catch global error handling with the error class.
3. Create a try/catch function in order to avoid duplicating code.
4. Distinguish development error and production error
